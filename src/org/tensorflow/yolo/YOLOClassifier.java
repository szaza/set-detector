...
public class YOLOClassifier {
    ...
    private final static int MAX_RECOGNIZED_CLASSES = 78;
    ...
    public List<Recognition> classifyImage(final float[] tensorFlowOutput, final Vector<String> labels) {
        ...
        Log.w(LOGGING_TAG, "NumClass: " + numClass);
        Log.w(LOGGING_TAG, "Size of queee: " + priorityQueue.size());
    ...
    private void calculateTopPredictions(final BoundingBox boundingBox, final PriorityQueue<Recognition> predictionQueue,
        ...
        for (int i=0; i<boundingBox.getClasses().length; i++) {
            ...
            for (int j=0; j<confidenceInClasses.length; j++) {
                ...
                if (confidenceInClasses[j] > THRESHOLD) {
                    predictionQueue.add(new Recognition(j, labels.get(j), (float) confidenceInClasses[j],
